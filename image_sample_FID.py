...
def get_graph(indx, g_true, ID_COLOR, draw_graph, save_svg):
    # build true graph
    G_true = nx.Graph()
    colors_H = [] #**** the filled color of nodes 
    node_size = []
    edge_color = []
    linewidths = []
    edgecolors = []
    # add nodes
    for k, label in enumerate(g_true[0]): #** g_true[0] contains room types
        _type = label
        if _type >= 0 and _type not in [11, 12]: 
              ...
              linewidths.append(3.0) #*** from zero to something
              #print(g_true)
              #print(g_true[0]) #*** to see what is happening inside it
    # add outside node
    ...
    # add edges
    for k, m, l in g_true[1]: #*** and g_true[1] should be the triples
        k = int(k)
        l = int(l)
        _type_k = g_true[0][k]
        _type_l = g_true[0][l]
...
def estimate_graph(indx, polys, nodes, G_gt, ID_COLOR, draw_graph, save_svg):
    ...
    rooms_inds = np.where((nodes != 11) & (nodes != 12) & (nodes != 18) & (nodes != 19) & (nodes != 20) & (nodes != 21) & (nodes != 22))[0] #********
...
def save_samples(
        ...
        save_gif=True, save_edges=False, #** save_gif to true
...
def main():
    ...
    for _ in range(5):
        ...
        if args.dataset=='rplan':
            ID_COLOR = {1: '#FF8C69', 2: '#b297f0', 3: '#FFD274', 4: '#BEBEBE', 5: '#BFE3E8',
                        #6: '#7BA779', #7: '#E87A90', 8: '#FF8C69', 10: '#1F849B', 
                        11: '#727171', 12: '#D3A2C7', 13: '#785A67', 18: '#7BA779', 19:'#8916cc', 20:'#0acff2', 21:'#3b3d47'} #**
            #O_ID_COLOR = {18: '#7BA779', 19:'#8916cc', 20:'#0acff2', 21:'#3b3d47'}
            num_room_types = 7 #***
            data = load_rplanhg_data(
                batch_size=args.batch_size,
                analog_bit=args.analog_bit,
                set_name=args.set_name, #*** here is exactly where the eval part of the rplanhg is read
        ...
        fid_score = calculate_fid_given_paths(['outputs/gt', 'outputs/pred'], 64, 'cuda', 2048) #changed 64 to 24 for the number of samples to match the command 
