...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, heads, dropout, activation):
        ...
        self.orient_attn = MultiHeadAttention(heads, d_model) #*** Orientation attention layer 
        self.ff = FeedForward(d_model, d_model*2, dropout, activation)
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, x, door_mask, self_mask, gen_mask, orient_mask): #****
        ...
        x = x + self.dropout(self.door_attn(x2,x2,x2,door_mask)) \
                ...
                + self.dropout(self.orient_attn(x2, x2, x2, orient_mask)) #**** adding the orinetation 
        x2 = self.norm_2(x)
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        self,
        in_channels,
        condition_channels, #** apparently the first time that condition_channels are defined 
        model_channels,
        out_channels,
        dataset,
        use_checkpoint,
        ...
        self.condition_emb = nn.Linear(self.condition_channels, self.model_channels) #nn. Linear(n,m) is a module that creates single layer feed forward network with n inputs and m output.
    ...
    def forward(self, x, timesteps, xtalpha, epsalpha, is_syn=False, **kwargs):
        ...
        if self.condition_channels>0:
            cond = None
            for key in [f'{prefix}room_types', f'{prefix}orient_types', f'{prefix}corner_indices', f'{prefix}room_indices']: #*** 
                if cond is None:
                    cond = kwargs[key]
                else:
        ...
        for layer in self.transformer_layers:
            out = layer(out, kwargs[f'{prefix}door_mask'], kwargs[f'{prefix}orient_mask'],  kwargs[f'{prefix}self_mask'], kwargs[f'{prefix}gen_mask']) #**** adding orientation mask
        ...
        if not self.analog_bit:
            ...
            out_bin = self.output_linear_bin2(out_bin, kwargs[f'{prefix}door_mask'], kwargs[f'{prefix}orient_mask'], kwargs[f'{prefix}self_mask'], kwargs[f'{prefix}gen_mask']) #** adding orientation mask
            out_bin = self.output_linear_bin3(out_bin, kwargs[f'{prefix}door_mask'], kwargs[f'{prefix}orient_mask'], kwargs[f'{prefix}self_mask'], kwargs[f'{prefix}gen_mask']) #** adding orientation mask
